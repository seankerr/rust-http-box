...
fn head() {
    ...
    assert_eq!(
        h.headers.get("cache-control").unwrap(),
        "max-age=0"
    );
    ...
    assert_eq!(
        h.headers.get("content-length").unwrap(),
        "101106"
    );

    assert_eq!(
        h.headers.get("content-type").unwrap(),
